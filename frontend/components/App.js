...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    case CREATE_QUOTE: return {...state,
    quotes: [...state.quotes, action.payload]}
    case DELETE_QUOTE: return {...state,
    quotes: state.quotes.filter(qt => {
      if(qt.id !== action.payload) return qt
        return {...qt, apocryphal: !qt.apocryphal}})}
    case EDIT_QUOTE_AUTHENTICITY: return {...state,
    quotes: state.quotes.map(qt => qt.id !== action.payload)}
    case SET_HIGHLIGHTED_QUOTE: return {...state,
    highlightedQuote: state.highlightedQuote === action.payload
    ? null: action.payload}
    case TOGGLE_VISIBILITY: return {...state,
...
export default function App() {
  ...
  const deleteQuote = id => {
    // 👇 implement
    dispatch({type: CREATE_QUOTE, payload: id})
  }
